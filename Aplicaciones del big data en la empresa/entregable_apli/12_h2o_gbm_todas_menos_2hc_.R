...
#-- De esta forma no usamos subvillage ni wpt_name
vars_gd <- numlev_df %>%
...
#--- Junto (cbind) los dos dataframes numericas y categoricas transformadas
train_end <- cbind(datcat_gd, datnum_df)
train_end$status_group <- as.factor(dattrainLabOr$status_group)

#----------  H2O ------------
#--- Modelo gbm - H2O - Todas menos las dos de high-cardinality
library(h2o)
h2o.init()
y <- "status_group"
...
h2o.performance(model = model_gbm, newdata = dattrain_test)

#-- Importancia Variables
var_imp <- h2o.varimp(model_gbm)
var_imp
#-- Grafico con importancia de variables
h2o.varimp_plot(model_gbm, 20)
...
#-- Hago predicción
my_pred <- as.data.table(h2o.predict(model_gbm, test_h2o))
...
# guardo submission
fwrite(my_sub, file = "./submissions/12_todas_menos2_h2ogbm.csv" )
