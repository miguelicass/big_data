...
# Salida: Modelo con numericas y unas cuantas categoricas (<100)
# Comentarios: 
#-------------------
...
for (i in 1:ncol(datcat_df)) {
  ...
  num_lev <- length(unique(datcat_df[, i]))
...
ggplot(impor_df, aes(fct_reorder(vars, Importance), Importance)) +
  ...
  theme_bw()
ggsave('./charts/01_evol_num_cat100.png')
...
# guardo submission
fwrite(my_sub, file = "./submissions/01_num_cat100.csv" )
...
#--- observaciones - longitud del dataframe,
# columns length
length(datcat_df[,1])
# rows length
length(datcat_df[1,])
...
head(datcat_df[, c('payment', 'payment_type')])
